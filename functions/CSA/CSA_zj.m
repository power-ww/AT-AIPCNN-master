...
function [fmin0,gPosition,cg_curve]=CSA_zj(searchAgents,iteMax,lb,ub,dim,fobj,I_MS,I_PAN,ratio,I_GT)
...
chameleonPositions(1,:)=[1,0.62,0.1,0.1,0.5];
chameleonPositions(2,:)=[1,0.62,0.1,0.1,0.707];
chameleonPositions(3,:)=[0.1,1,0.62,0.1,0.5];
chameleonPositions(4,:)=[0.1,1,0.62,0.1,0.707];

%  chameleonPositions=repmat(A1,[searchAgents,1]);
...
for i=1:searchAgents
     fit(i,1)=fobj(chameleonPositions(i,:),I_MS,I_PAN,ratio,I_GT);
end
...
%% Visualize the results
   cg_curve(t)=fmin0; % Best found value until iteration t
    if t>=10&abs(cg_curve(t)-((cg_curve(t-3)+cg_curve(t-2)+cg_curve(t-1))/3))<=0.00001
        flag=0;
    end
   t=t+1;
end
...
fmin0 =fobj(g_best,I_MS,I_PAN,ratio,I_GT);
